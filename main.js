...
document.getElementById("checkin-form").addEventListener("submit", function(e) {
  ...
  // Flatten children info into one string for the email
...
document.getElementById("print-pdf-btn").addEventListener("click", function () {
  ...
  const { parentEmail, children, familyCode } = lastCheckinData;
  ...
  children.forEach((c, idx) => {
    doc.text(`Child ${idx + 1}: ${c.name}`, 15, 90 + idx * 20);
    doc.text(`Age: ${c.age}`, 80, 90 + idx * 20);
    doc.text(`Allergies/Notes: ${c.notes || '-'}`, 15, 97 + idx * 20);
  });

  const yOffset = 90 + children.length * 20;
  doc.setFontSize(16);
